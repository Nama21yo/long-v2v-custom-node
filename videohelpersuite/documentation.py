...
descriptions = {
  ...
  "VHS_SplitLatents": ['Split Latents 🎥🅥🅗🅢', short_desc('Split a set of latents into two groups'),
    ...
    'VHS_FrameByFrameProcessor': [
        'Frame-by-Frame Video Processor 🎥🅥🅗🅢',
        short_desc('Process videos frame-by-frame with automatic batch progression'),
        {
            'Inputs': {
                'video_path': 'Path to the video file to process',
                'frames_per_batch': 'Number of frames to process in each execution (default: 1)',
                'force_rate': 'Force a specific frame rate (0 = use source rate)',
                'custom_width': 'Custom width for output frames (0 = keep original)',
                'custom_height': 'Custom height for output frames (0 = keep original)',
                'auto_continue': 'Automatically queue next batch when current completes',
                'vae': '(optional) VAE to encode frames as latents',
                'reset_counter': 'Reset frame counter to start from beginning',
            },
            'Outputs': {
                'frames': 'The current batch of frames as images',
                'latents': 'The current batch of frames as latents (if VAE provided)',
                'current_frame_index': 'Index of the first frame in current batch',
                'batch_count': 'Current batch number (starts at 1)',
                'frames_remaining': 'Number of frames left to process',
                'is_complete': 'Boolean indicating if all frames processed',
                'video_info': 'Information about the video',
                'audio': 'Audio from the video',
            },
            'Widgets': {
                'video_path': 'Path to video file. Supports webm, mp4, mkv, gif, mov formats',
                'frames_per_batch': 'Process this many frames per execution. Set to 1 for true frame-by-frame processing',
                'auto_continue': 'When enabled, automatically queues the next batch until all frames are processed',
                'reset_counter': 'Check this to start processing from frame 0 again',
            },
        }
    ],
  ...
  "VHS_SelectLatest": ['Select Latest 🎥🅥🅗🅢', short_desc('Experimental virtual node to select the most recently modified file from a given folder'),
    ...
    'VHS_VideoMerger': [
        'Video Merger 🎥🅥🅗🅢',
        short_desc('Merge multiple video files from a directory with intelligent trimming'),
        {
            'Description': [
                'Combines multiple video chunks into a single video file',
                'Perfect for merging frame-by-frame processed videos',
                'Supports intelligent trimming to remove overlap between chunks',
                'Can add audio track from external source',
            ],
            'Inputs': {
                'folder_path': 'Relative path to folder containing MP4 files (relative to output directory)',
                'output_filename': 'Name for the merged video (without .mp4 extension)',
                'fps': 'Frame rate used for trimming calculations',
                'chunk_size': 'Original size of each video chunk in frames',
                'overlap_frames': 'Total overlapping frames between chunks (split evenly: half from end of A, half from start of B)',
                'audio_source': '(optional) Path to audio file to add to merged video',
                'save_output': '(optional) Save to output directory (False = temp directory)',
            },
            'Outputs': {
                'merged_video_path': 'Full path to the merged video file',
                'filenames': 'VHS_FILENAMES object for compatibility with other VHS nodes',
            },
            'Widgets': {
                'folder_path': [
                    'Enter the folder containing your video chunks',
                    'Path is relative to ComfyUI output directory',
                    'Example: "my_project/chunks" will look in output/my_project/chunks/',
                ],
                'overlap_frames': [
                    'Total number of overlapping frames between consecutive chunks',
                    'The overlap is split evenly: half from end of clip A, half from start of clip B',
                    'Example: overlap_frames=20 means 10 frames cut from end of A and 10 from start of B',
                    'First chunk: no start cut, only end cut applied',
                    'Last chunk: only start cut, no end cut applied',
                    'Use this when chunks have duplicate frames to create seamless transitions',
                ],
                'audio_source': [
                    'Path to audio file (MP4, WAV, MP3, etc.)',
                    'Can be absolute path or relative to input directory',
                    'Leave empty for silent video',
