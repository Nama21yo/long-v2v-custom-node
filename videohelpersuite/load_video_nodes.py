...
import gc
import os
import comfy.model_management  # Internal ComfyUI memory manager

class LoadVideoSingleton:
    """
    Loads a SINGLE video file into a tensor. 
    Includes aggressive garbage collection between sequential executions.
    """
    ...
    def load_video_safe(self, video_path, frame_limit):

        # --- PHASE 1: CLEANUP/RELEASE MEMORY ---
        ...
        print(f"[Memory Guard] Memory sanitized. Loading: {os.path.basename(video_path)}")

        if not os.path.exists(video_path):
            raise FileNotFoundError(f"Chunk file missing: {video_path}")
        ...
        try:
            ...
            # limit how many frames we load
            total_frames = len(vr)
            if frame_limit > 0 and frame_limit < total_frames:
                total_frames = frame_limit

            # --- GET DIMENSIONS FROM FIRST FRAME ---
            # Extract a small frame to inspect shape (C, H, W)
            sample = vr[0]
            if sample.ndim == 3:
                height = sample.shape[1]
                width  = sample.shape[2]
            else:
                raise RuntimeError("Unsupported frame shape from Decord")

            # Estimate RAM usage
            bytes_per_frame = width * height * 3 * 4  # float32 RGB
            est_size_gb = (bytes_per_frame * total_frames) / (1024 ** 3)
            print(f"[Memory Guard] Estimated RAM usage ~ {est_size_gb:.2f} GB")
            ...
            images = tensor_batch.float().div_(255.0)
            ...
            print(f"[LoadVideoSingleton] Failed to load: {e}")
            raise

        return (images, total_frames, fps)
